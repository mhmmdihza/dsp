...
public class RegisterControllerTest {
	...
	public void testRegister() {
		...
		sc.add(this.setScenario("name exceeded limit",
				...
				new ArrayList<>(Arrays.asList("name-size must be between 0 and 60")), HttpStatus.OK));
		// scenario 4
		...
		for (RegisterScenario r : sc) {
			...
			try {
				...
				if (r.getErrorValidation().size() == 0) {
					ResponseEntity<Void> resp = controller.registerUser(r.getRequest());
	...
	private RegisterScenario setScenario(String desc, UserRegistrationRequest req, List<String> errorValidation,
		...
		return new RegisterScenario(errorValidation, desc, req, expected);
	}
}
